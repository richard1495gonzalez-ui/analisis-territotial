@echo off
:: ==============================================================================
:: AUTOMATIZACIÓN DE ENTORNO Y EJECUCIÓN - ANÁLISIS TERRITORIAL
:: ==============================================================================
...
cls
echo ==============================================================================
echo        INICIANDO AUTOMATIZACION: ANALISIS TERRITORIAL
echo ==============================================================================
echo.

:: 2. Generación de Estructura de Arquitectura (MVC)
echo === [1/6] Comprobando estructura del proyecto (MVC)...
...
:: 3. Gestión de Python y Dependencias
echo === [2/6] Gestionando Entorno Virtual (venv) de Python...
if not exist "venv" (
    echo   [+] No se encontro entorno virtual. Creando venv...
    python -m venv venv
) else (
    echo   [i] Entorno virtual existente detectado.
)
echo   [+] Instalando/Verificando dependencia mysql-connector-python...
venv\Scripts\pip.exe install mysql-connector-python
echo.

:: 4. Infraestructura Docker
echo === [3/6] Levantando infraestructura Docker...
:: Validar si hay colisiones en puerto 3306
netstat -o -an | findstr /R "\<3306\>" >nul 2>&1
if %errorlevel% equ 0 (
    echo   [!] ADVERTENCIA: El puerto 3306 esta en uso. Si tienes un MySQL de XAMPP local ejecutando,
    echo       esto podria causar conflicto con el contenedor Docker.
    echo       Asegurate de detener el MySQL local si experimentas problemas de conexion.
)
echo   [+] Iniciando contenedores (docker compose up -d)...
docker compose up -d
echo.

:: Copia del CSV municipios.csv al directorio estricto
echo === [4/6] Configurando archivo municipios.csv para importacion...
if not exist "C:\xampp\mysql\Taller_Municipios" (
    echo   [+] Creando carpeta C:\xampp\mysql\Taller_Municipios...
    mkdir "C:\xampp\mysql\Taller_Municipios"
)
echo   [+] Copiando municipios.csv a C:\xampp\mysql\Taller_Municipios...
copy /Y "municipios.csv" "C:\xampp\mysql\Taller_Municipios\municipios.csv"
echo.

:: Esperar establemente a que el servicio MySQL esté activo
echo === [5/6] Esperando a que el servicio MySQL de Docker este listo...
:wait_mysql
...
echo   [+] Ejecutando Crear base de datos.sql...
"C:\xampp\mysql\bin\mysql.exe" -h 127.0.0.1 -P 3306 -u root < "Crear base de datos.sql"

echo   [+] Ejecutando Cargue de datos.sql (con local-infile habilitado)...
"C:\xampp\mysql\bin\mysql.exe" -h 127.0.0.1 -P 3306 -u root --local-infile=1 < "Cargue de datos.sql"
echo.

:: 5. Consultas en Consola
echo === [6/6] Ejecutando Consultas.sql directamente en Consola...
echo ------------------------------------------------------------------------------
"C:\xampp\mysql\bin\mysql.exe" -h 127.0.0.1 -P 3306 -u root < "Consultas.sql"
echo ------------------------------------------------------------------------------
echo.

:: 6. Herramientas y Ejecución Final
echo === Apertura de herramientas y Ejecucion Final ===
echo   [+] Abriendo Visual Studio Code...
start "" code .

echo   [+] Ejecutando programa principal Python MVC desde el venv...
echo.
